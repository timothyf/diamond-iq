class PitchDataSyncTaskLauncher
  ...
  def call
    ...
    task_run = AdminTaskRun.create!(task_name: TASK_NAME, task_parameters: attributes, total_items: estimate.fetch(:chunk_count))
    enqueued_job = PitchDataSyncJob.perform_later(task_run.id)
    raise ActiveJob::EnqueueError, "Statcast pitch data synchronization could not be enqueued" unless enqueued_job
